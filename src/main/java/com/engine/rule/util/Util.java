...
public class Util {
    ...
    public static Set<String> getInvalidData(List<SignalData> signalDatum, Map<String, List<Rule>> ruleMap) {
        Set<String> invalidDatum = new HashSet<>();
        FilterPredicate filterPredicate = new FilterPredicate(ruleMap);
        signalDatum.stream().filter(filterPredicate::test)
                .forEach(signalData -> invalidDatum.add(signalData.getSignal()));
        return invalidDatum;
    }
